...
class AccountValidatorTest {
	 ...
	 * @ParameterizedTest
	 * 
	 * @ValueSource(doubles = {0.0, 0.2, 0.4}) public void testValidPercent(double
	 * percent) { Account validAccount =
	 * Account.builder().balance(OK_BALANCE).percent(percent).email(OK_EMAIL).build(
	...
	public void testValidPercent(Double percent) {
		Account validAccount = Account.builder().balance(OK_BALANCE).percent(percent)
				.email(OK_EMAIL).build();
		Errors errors = new BeanPropertyBindingResult(validAccount, "account");
		accountValidator.validate(validAccount, errors);
		assertThat(errors.getAllErrors()).isEmpty();
	}
	...
	public void testInvalidPercent(double percent) {
		Account invalidAccount = Account.builder().balance(OK_BALANCE).percent(percent)
				.email(OK_EMAIL).build();
